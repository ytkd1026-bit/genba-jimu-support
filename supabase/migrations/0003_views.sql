...
-- ─── 案件別損益（全KPIの土台） ───────────────────────────
create or replace view v_project_profit with (security_invoker = true) as
select
  p.organization_id,
  p.id                as project_id,
  p.project_number,
...
create or replace view v_monthly_profit with (security_invoker = true) as
...
-- ─── 顧客別売上・粗利（A-1 / A-2） ───────────────────────
create or replace view v_customer_profit with (security_invoker = true) as
select
  pp.organization_id,
  pp.customer_id,
  coalesce(c.name, nullif(pp.client_name, ''), '（未紐付け）') as customer_name,
...
group by pp.organization_id, pp.customer_id, coalesce(c.name, nullif(pp.client_name, ''), '（未紐付け）');

-- ─── 流入経路別（売上・粗利・件数） ──────────────────────
create or replace view v_inflow_profit with (security_invoker = true) as
select
  organization_id,
...
-- ─── 元請別（流入経路=元請けの取引先別） ─────────────────
create or replace view v_contractor_profit with (security_invoker = true) as
select
  organization_id,
...
-- ─── 経路別成約率（CARE: 相談→案件化） ───────────────────
create or replace view v_inflow_conversion with (security_invoker = true) as
select
  organization_id,
  channel,
...
-- ─── 未請求案件（請求もれゼロ運用） ──────────────────────
create or replace view v_uninvoiced_projects with (security_invoker = true) as
...
create or replace view v_decisions_due with (security_invoker = true) as
