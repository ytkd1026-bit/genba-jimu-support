  ...
  return (
    <div className={`overflow-hidden rounded-2xl border shadow-sm ${STATUS_CARD_STYLE[project.billingStatus]}`}>
      ...
      <div className="flex items-center justify-between border-b border-[var(--nu-border-soft)] bg-[var(--nu-bg)] px-4 py-2.5">
        <div className="flex items-center gap-2">
          ...
          {project.isBulkCandidate && project.billingStatus === "unbilled" && (
            <span className="rounded-full bg-[var(--nu-primary)]/10 px-2 py-0.5 text-xs font-bold text-[var(--nu-primary)]">
              一括請求候補
            </span>
          )}
        </div>
      ...
      <div className="px-4 py-3 space-y-1.5">
        <p className="text-base font-bold text-[var(--nu-text)] leading-tight">{project.projectName}</p>
        <p className="text-xs text-slate-400">{project.clientName}　{project.siteAddress}</p>
        <p className="text-xs text-slate-500">{project.workContent}</p>
        <div className="flex flex-wrap gap-x-4 gap-y-0.5">
          <span className="text-xs text-slate-400">完了日：{project.completedAt}</span>
          <span className="text-xs text-slate-400">支払条件：{project.paymentTerms}</span>
          {project.invoicedAt && (
            <span className="text-xs text-slate-400">請求日：{project.invoicedAt}</span>
          )}
        </div>
        {/* 金額（目立つ表示） */}
        <div className="rounded-xl bg-[var(--nu-primary-bg)] px-3 py-2 text-right">
          <p className="text-xs text-[var(--nu-primary)]">税込金額</p>
          <p className="text-xl font-bold text-[var(--nu-primary)]">{fmtYen(project.totalAmount)}</p>
        </div>
      </div>

      {/* アクションボタン */}
      <div className="grid grid-cols-3 gap-1.5 border-t border-[var(--nu-border-soft)] px-4 py-3">
        <button
          type="button"
          onClick={() => onSingleInvoice(project.id)}
          className="flex items-center justify-center rounded-xl bg-[var(--nu-primary)] px-2 py-2.5 text-center text-xs font-bold text-white active:opacity-80"
        >
          単体請求書<br />を作る
        </button>
        <button
          ...
          className="flex items-center justify-center rounded-xl border border-[var(--nu-primary)] bg-white px-2 py-2.5 text-center text-xs font-bold text-[var(--nu-primary)] active:opacity-80"
        >
          一括請求<br />に含める
        </button>
        <Link
          href="/new/projects"
          className="flex items-center justify-center rounded-xl border border-[var(--nu-border)] bg-white px-2 py-2.5 text-center text-xs font-bold text-slate-600 active:opacity-80"
        >
          案件<br />を見る
        </Link>
      </div>
...
export default function UnbilledPage() {
  ...
  function handleSingleInvoice(id: string) {
    ...
    router.push("/new/invoices/single");
  }
  ...
  const inputCls =
    "w-full rounded-xl border border-[var(--nu-border)] bg-[var(--nu-bg)] px-3 py-2.5 text-sm text-[var(--nu-text)] placeholder:text-slate-300 focus:border-[var(--nu-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--nu-primary)]/30";
  const selectCls =
    "w-full rounded-xl border border-[var(--nu-border)] bg-[var(--nu-bg)] px-3 py-2.5 text-sm text-[var(--nu-text)] focus:border-[var(--nu-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--nu-primary)]/30";

  return (
    <div className="">
      <PageHeader title="未請求確認" subtitle="請求漏れを確認する" back="/new/invoices" />
      <div className="px-4 py-4">
        ...
        <div className="space-y-3">
          ...
          {/* 集計カード */}
          <div className="rounded-2xl bg-[var(--nu-primary)] p-4 shadow-sm text-white">
          ...
          {!isDemo && invoiceProjects.length === 0 && (
            <div className="rounded-2xl border-2 border-dashed border-[var(--nu-border)] py-10 text-center">
              <p className="text-sm text-slate-500">未請求案件はありません。</p>
              <p className="mt-1.5 text-sm text-slate-500">案件登録・見積作成後、請求書作成を試してください。</p>
            </div>
          )}
          ...
          {unbilledList.length > 0 && (
            <section>
              <h2 className="mb-2 px-1 text-sm font-bold text-[var(--nu-text)]">
                未請求
          ...
          {isDemo && unbilledList.length === 0 && bulkSelectedList.length === 0 && singleInvoicedList.length === 0 && (
            <div className="rounded-2xl border-2 border-dashed border-[var(--nu-border)] py-10 text-center">
              <p className="text-sm text-slate-400">該当する案件はありません</p>
            </div>
          )}
