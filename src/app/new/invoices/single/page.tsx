...
const inputCls = "w-full rounded-xl border border-[var(--nu-border)] bg-white px-3 py-2.5 text-sm text-[var(--nu-text)] focus:border-[var(--nu-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--nu-primary)]/20";
...
export default function SingleInvoicePage() {
  ...
  async function handlePDF() {
    ...
    try {
      ...
      const { makeSingleInvoicePDF } = await import("@/app/projects/sample/single-invoice/SingleInvoicePDF");
  ...
  return (
    <div className="">
      <PageHeader title="単体請求書" subtitle="1案件ごとの請求書を作る" back="/new/invoices" />
      <div className="px-4 py-4">
        ...
        <div className="space-y-4">
                ...
                {filteredProjects.length === 0 ? (
                  ...
                  filteredProjects.map((p) => (
                    <div key={p.id} className="rounded-xl border border-[var(--nu-border-soft)] bg-[var(--nu-bg)] p-3 space-y-1.5">
                      <div>
                        <p className="text-xs text-slate-400">{p.date}　{p.clientName}</p>
                        <p className="text-sm font-bold text-[var(--nu-text)] leading-tight">{p.projectName}</p>
                        <p className="text-xs text-slate-500">{p.siteAddress}</p>
                        <p className="text-xs text-slate-400">{p.workContent}</p>
                        <p className="text-xs text-slate-400">完了日：{p.completedAt.replace(/-/g, "/")}</p>
                        <p className="mt-1 text-sm font-bold text-[var(--nu-primary)]">{fmtYen(p.totalAmount)}</p>
                      </div>
                      <button type="button"
                        onClick={() => {
          ...
          {/* ── 2. 請求日・支払期日（コンパクト編集） ── */}
          <div className="rounded-2xl bg-white p-4 shadow-sm space-y-3">
            <h2 className="border-b border-[var(--nu-border-soft)] pb-2 text-sm font-bold text-[var(--nu-text)]">請求日設定</h2>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="mb-1 block text-xs text-slate-400">請求日</label>
                <input type="date" value={invoiceDate} onChange={(e) => setInvoiceDate(e.target.value)} className={inputCls} />
              </div>
              <div>
                <label className="mb-1 block text-xs text-slate-400">支払期日</label>
              ...
              <ul className="space-y-2">
                {[
                  ...
                  <li key={item.label} className="flex items-start gap-2 text-sm">
                    <span className="w-20 shrink-0 pt-0.5 text-xs text-slate-400">{item.label}</span>
