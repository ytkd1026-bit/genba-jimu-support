...
const CAL_BADGE_STYLE: Record<CalBadge, string> = {
  ...
  施工: "bg-[var(--nu-primary)]/10 text-[var(--nu-primary)]",
...
const STATUS_STYLE: Record<EventStatus, string> = {
  予定:   "bg-[var(--nu-bg)] text-slate-600",
...
export default function SchedulePage() {
  ...
  return (
    <div className="">
      <PageHeader title="スケジュール" subtitle="施工・材料の予定を確認" back="/new" />
      <div className="px-4 py-4">
        ...
        <div className="space-y-3">
          ...
          <div className="overflow-hidden rounded-2xl bg-white shadow-sm">
            ...
            {/* 月ナビゲーション */}
            <div className="flex items-center justify-between border-b border-[var(--nu-border-soft)] px-4 py-3">
              <button type="button"
                onClick={() => showInfo("月移動は次工程で追加します。")}
                className="rounded-xl border border-[var(--nu-border)] px-3 py-1.5 text-sm font-bold text-slate-600 active:opacity-70">
                ← 前月
              </button>
              <span className="text-base font-bold text-[var(--nu-text)]">{CALENDAR_MONTH_LABEL}</span>
              <button type="button"
                onClick={() => showInfo("月移動は次工程で追加します。")}
                className="rounded-xl border border-[var(--nu-border)] px-3 py-1.5 text-sm font-bold text-slate-600 active:opacity-70">
                翌月 →
            ...
            {/* 曜日ヘッダー */}
            <div className="grid grid-cols-7 border-b border-[var(--nu-border-soft)] bg-[var(--nu-bg)]">
              {WEEKDAYS.map((wd, i) => (
            ...
            <div className="grid grid-cols-7">
              {CALENDAR_CELLS.map((day, idx) => {
                if (day === null) {
                  return (
                    <div key={`pad-${idx}`}
                      className="min-h-[58px] border-b border-r border-stone-50 bg-[var(--nu-bg)]/40" />
                  );
                }
                ...
                return (
                  <button
                    ...
                    className={`flex min-h-[58px] w-full flex-col items-center gap-0.5 border-b border-r border-[var(--nu-border-soft)] px-0.5 py-1.5 transition-colors active:opacity-80 ${
                      isSelected
                        ? "bg-[var(--nu-primary)]"
                        : "hover:bg-[var(--nu-primary-bg)]/60"
                    }`}
                  >
                    {/* 日付数字 */}
            ...
            {/* 凡例 */}
            <div className="flex flex-wrap items-center gap-1.5 border-t border-[var(--nu-border-soft)] px-4 py-2.5">
              <span className="text-[10px] text-slate-400 shrink-0">凡例：</span>
            ...
            {selectedDay === null ? (
              ...
              <div className="space-y-2.5">
                ...
                {selectedEvents.map((ev) => (
                  <div key={ev.id} className="rounded-xl border border-[var(--nu-border-soft)] bg-[var(--nu-bg)] p-3">
                    ...
                    <p className="mb-0.5 text-sm font-bold text-[var(--nu-text)] leading-tight">{ev.projectName}</p>
                    <p className="text-xs text-slate-400">{ev.clientName}</p>
                    {ev.siteAddress !== "-" && (
                      <p className="text-xs text-slate-400">{ev.siteAddress}</p>
                    )}
                    <div className="mt-2">
                      <Link href="/new/projects"
                        className="inline-flex items-center gap-1 rounded-lg bg-[var(--nu-primary)]/10 px-3 py-1.5 text-xs font-bold text-[var(--nu-primary)] active:opacity-70">
                        案件を見る →
                      </Link>
                    </div>
                  </div>
                ...
                {selectedAlerts.map((al) => (
                  <div key={al.id} className={`rounded-xl p-3 ${materialAlertCardStyle(al)}`}>
                    ...
                    <p className="mb-0.5 text-sm font-bold text-[var(--nu-text)]">{al.projectName}</p>
                    <p className="mb-1 text-xs text-slate-500">{al.contractorName}</p>
          ...
          {/* ── 材料発注アラート ── */}
          <div className="rounded-2xl bg-white p-4 shadow-sm">
            <h2 className="mb-0.5 border-b border-[var(--nu-border-soft)] pb-2 text-sm font-bold text-[var(--nu-text)]">
              🔔 材料発注アラート
            </h2>
            ...
            {materialAlerts.length > 0 ? (
                ...
                {materialAlerts.map((al) => (
                  <div key={al.id} className={`rounded-xl p-3 ${materialAlertCardStyle(al)}`}>
                    <div className="mb-1.5 flex flex-wrap items-center gap-1.5">
                      ...
                      {al.alertDate !== "-" && (
                        <span className="ml-auto text-xs text-slate-500">⏰ {al.alertDate}</span>
                      )}
                    </div>
                    <p className="mb-1 text-sm font-bold text-[var(--nu-text)] leading-tight">{al.projectName}</p>
                    <p className="mb-0.5 text-xs text-slate-400">{al.contractorName}</p>
                    <div className="mb-1.5 flex flex-wrap gap-x-3 gap-y-0.5">
                      <span className="text-xs text-slate-500">施工日：{al.constructionDate}</span>
                      <span className="text-xs text-slate-500">搬入予定：{al.materialDeliveryDate}</span>
                    </div>
              ...
              <div className="rounded-xl border-2 border-dashed border-[var(--nu-border)] py-8 text-center">
                <p className="text-sm text-slate-500">登録された予定はありません。</p>
                <p className="mt-1.5 text-sm text-slate-500">案件登録後、施工予定や材料発注予定を確認できます。</p>
              </div>
            )}
          </div>

          {/* ── カレンダー連携について ── */}
          <div className="rounded-2xl bg-[var(--nu-bg)] p-4 shadow-sm ring-1 ring-[var(--nu-border)]">
            <h3 className="mb-1.5 text-sm font-bold text-slate-600">📅 カレンダー連携について</h3>
            ...
            <button
              ...
              className="w-full rounded-xl border border-[var(--nu-border)] bg-white py-2.5 text-sm font-bold text-slate-600 shadow-sm active:opacity-70"
            >
              カレンダー連携設定
            </button>
          </div>
