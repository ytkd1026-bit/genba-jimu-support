...
function IdToggleChips({
  ...
  return (
    <div>
      ...
      {options.length === 0 ? (
        <p className="rounded-lg bg-[var(--nu-bg)] px-3 py-2.5 text-xs text-slate-400">{emptyHint}</p>
      ) : (
              ...
              <button
                ...
                className={`min-h-[44px] rounded-xl px-3 py-2 text-xs font-bold active:opacity-80 ${
                  active
                    ...
                    : "border border-[var(--nu-border)] bg-white text-slate-500"
                }`}
              >
                {opt.caption}
              </button>
...
export default function SurveyPage() {
  ...
  if (notFound) {
    return (
      <div className="">
        <PageHeader title="現地調査" subtitle="案件が見つかりません" back="/new/projects" />
  ...
  return (
    <div className="pb-4">
      <PageHeader title="現地調査" subtitle="被害・問題を1件ずつ記録" back={`/new/projects/${encodeURIComponent(projectId)}`} />
      <div className="px-4 py-4">

        <NuProjectHeader project={project} />
        <NuProjectTabs projectId={projectId} active="survey" />
        ...
        <div className="space-y-3">
          {records.length === 0 && (
            <div className="rounded-2xl border-2 border-dashed border-[var(--nu-border)] px-4 py-10 text-center">
              <p className="text-sm text-slate-500">まだ被害記録がありません。</p>
              <p className="mt-1.5 text-sm text-slate-500">「被害を追加する」から記録を始めてください。</p>
            </div>
          )}

          {records.map((r, index) => (
            <div key={r.damageId} className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-[var(--nu-border-soft)]">
              {/* カードヘッダー */}
              <div className="flex items-center justify-between border-b border-[var(--nu-border-soft)] bg-[var(--nu-bg)] px-4 py-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-slate-500">被害 {index + 1}</span>
                </div>
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => duplicateRecord(r.damageId)}
                    className="min-h-[44px] px-1 text-xs text-slate-400 active:text-slate-600">
                    複製
                  </button>
                  <button type="button" onClick={() => removeRecord(r.damageId)}
                    className="min-h-[44px] px-1 text-xs text-slate-400 active:text-red-500">
                    削除
                  </button>
                </div>
              </div>
        ...
        <button
          type="button"
          onClick={addRecord}
          className="mt-3 flex min-h-[52px] w-full items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-[var(--nu-primary)]/40 bg-white px-4 py-3 text-sm font-bold text-[var(--nu-primary)] active:opacity-80"
        >
          ＋ 被害を追加する
        </button>
        ...
        {/* ── 現地調査報告書PDF ───────────────────────────── */}
        {records.length > 0 && (
          <div className="mt-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-[var(--nu-border-soft)]">
            <h2 className="mb-2 text-sm font-bold text-[var(--nu-text)]">現地調査報告書PDF</h2>
            <StructuredTextInput
            ...
            <button
              ...
              className="mt-3 flex min-h-[52px] w-full items-center justify-center rounded-2xl border border-[var(--nu-primary)] bg-white px-4 py-3 text-sm font-bold text-[var(--nu-primary)] active:opacity-80 disabled:opacity-50"
            >
