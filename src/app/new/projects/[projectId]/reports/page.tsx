...
export default function WorkReportsPage() {
  ...
  if (notFound) {
    return (
      <div className="">
        <PageHeader title="作業報告" subtitle="案件が見つかりません" back="/new/projects" />
  ...
  return (
    <div className="pb-4">
      <PageHeader title="作業報告" subtitle="作業日ごとの報告とPDF" back={`/new/projects/${encodeURIComponent(projectId)}`} />
      <div className="px-4 py-4">

        <NuProjectHeader project={project} />
        <NuProjectTabs projectId={projectId} active="reports" />
        ...
        <div className="space-y-3">
          {records.length === 0 && (
            <div className="rounded-2xl border-2 border-dashed border-[var(--nu-border)] px-4 py-10 text-center">
              <p className="text-sm text-slate-500">まだ作業報告がありません。</p>
              <p className="mt-1.5 text-sm text-slate-500">「作業報告を追加する」から記録を始めてください。</p>
            </div>
          )}

          {records.map((r) => (
            <div key={r.reportId} className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-[var(--nu-border-soft)]">
              ...
              <div className="space-y-3 p-3">
                ...
                <div>
                  ...
                  {photos.length === 0 ? (
                    <p className="rounded-lg bg-[var(--nu-bg)] px-3 py-2.5 text-xs text-slate-400">
                ...
                <button
                  ...
                  className="flex min-h-[44px] w-full items-center justify-center rounded-xl border border-[var(--nu-primary)] bg-white px-3 py-2 text-xs font-bold text-[var(--nu-primary)] active:opacity-80 disabled:opacity-50"
                >
