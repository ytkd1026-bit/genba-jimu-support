...
import { nuInput as fldInput, nuLbl as lbl } from "@/app/new/_lib/formStyles";
...
export default function ProjectInvoicePage() {
  ...
  if (notFound) {
    return (
      <div className="">
        <PageHeader title="請求書" subtitle="案件が見つかりません" back="/new/projects" />
        <div className="px-4 py-10 text-center">
          ...
          <Link href="/new/projects" className="mt-4 inline-flex min-h-[44px] items-center justify-center rounded-xl bg-[var(--nu-primary)] px-5 py-2.5 text-sm font-bold text-white active:opacity-80">
            案件一覧へ戻る
          </Link>
        </div>
      </div>
  ...
  if (!project) return <div className="" />;

  return (
    <div className="pb-4">
      <PageHeader title="請求書" subtitle="工事項目から請求書を作成" back={`/new/projects/${encodeURIComponent(projectId)}`} />
      <div className="px-4 py-4">

        <NuProjectHeader project={project} />
        <NuProjectTabs projectId={projectId} active="invoice" />

        <div className="mb-3 flex items-center justify-between rounded-xl bg-white px-3 py-2 text-xs ring-1 ring-[var(--nu-border-soft)]">
          <span className="text-slate-400">請求番号</span>
          <span className="font-mono font-bold text-[var(--nu-text)]">{docNumber}</span>
        </div>
        ...
        {/* 請求情報入力 */}
        <div className="mb-3 space-y-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-[var(--nu-border-soft)]">
        ...
        {workItems.length === 0 && mode === "edit" ? (
          <div className="rounded-2xl border-2 border-dashed border-[var(--nu-border)] px-4 py-10 text-center">
            <p className="text-sm text-slate-500">工事項目がありません。</p>
            <Link href={`/new/projects/${encodeURIComponent(projectId)}/work-items`}
              className="mt-3 inline-flex min-h-[44px] items-center justify-center rounded-xl bg-[var(--nu-primary)] px-5 py-2.5 text-sm font-bold text-white active:opacity-80">
              工事項目・原価へ
            </Link>
          </div>
        ) : (
          <>
            ...
            {mode === "edit" ? (
                ...
                {workItems.map((w) => {
                  ...
                  return (
                    <div key={w.workItemId}
                      className={`rounded-xl p-3 shadow-sm ring-1 ${isExcluded ? "bg-[var(--nu-bg)] ring-[var(--nu-border)] opacity-60" : "bg-white ring-[var(--nu-border-soft)]"}`}>
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <p className="text-sm font-bold text-[var(--nu-text)]">{w.workName || "（工事名未入力）"}</p>
                          <p className="text-xs text-slate-400">{w.quantity} {w.unit} × {fmtYen(w.sellingUnitPrice)}</p>
                        </div>
                        <div className="shrink-0 text-right">
                          <p className="text-sm font-bold text-[var(--nu-text)]">{fmtYen(w.sellingAmount)}</p>
                          <button type="button" onClick={() => toggleExclude(w.workItemId)}
                            className={`mt-1 min-h-[36px] rounded-lg px-2 py-1 text-xs font-bold active:opacity-80 ${isExcluded ? "bg-[var(--nu-primary)] text-white" : "border border-[var(--nu-border)] text-slate-500"}`}>
              ...
              <div className="space-y-2">
                {sellingLines.map((l) => (
                  <div key={l.workItemId} className="rounded-xl bg-white p-3 shadow-sm ring-1 ring-[var(--nu-border-soft)]">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="text-sm font-bold text-[var(--nu-text)]">{l.workName || "（工事名未入力）"}</p>
                        <p className="text-xs text-slate-400">
                          {l.quantity} {l.unit} × {fmtYen(l.sellingUnitPrice)}・
                          {l.taxType === "taxable" ? `課税${l.taxRate}%` : l.taxType === "non_taxable" ? "非課税" : "不課税・対象外"}
                        </p>
                      </div>
                      <p className="shrink-0 text-sm font-bold text-[var(--nu-text)]">{fmtYen(l.sellingAmount)}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <button type="button" onClick={handleSave}
              className="mt-3 flex min-h-[52px] w-full items-center justify-center rounded-2xl bg-[var(--nu-primary)] px-4 py-3 text-sm font-bold text-white shadow-sm active:opacity-80">
              請求書を保存する（案件に紐付け）
            </button>
            <button type="button" disabled={pdfLoading} onClick={() => void handlePdf()}
              className="mt-2 flex min-h-[52px] w-full items-center justify-center rounded-2xl border border-[var(--nu-primary)] bg-white px-4 py-3 text-sm font-bold text-[var(--nu-primary)] active:opacity-80 disabled:opacity-50">
              {pdfLoading ? "PDF作成中..." : "📄 請求書PDFを保存して発行"}
            </button>
            {mode === "view" && (
              <button type="button" onClick={recreateFromWorkItems}
                className="mt-2 flex min-h-[52px] w-full items-center justify-center rounded-2xl border border-[var(--nu-border)] bg-white px-4 py-3 text-sm font-bold text-slate-600 active:opacity-80">
                現在の工事項目から請求書を再作成
              </button>
            )}
          </>
