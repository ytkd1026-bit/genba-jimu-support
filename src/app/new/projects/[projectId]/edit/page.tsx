...
import { nuInput as fldInput, nuSelect as fldSelect, nuReadOnly as readOnlyFld, nuLbl as lbl } from "@/app/new/_lib/formStyles";
...
export default function ProjectDetailPage() {
  ...
  if (notFound) {
    return (
      <div className="">
        <PageHeader title="案件情報の編集" subtitle="案件が見つかりません" back="/new/projects" />
  ...
  return (
    <div className="pb-4">
      <PageHeader title="案件情報の編集" subtitle="現場・元請・住所を更新" />
      <div className="px-4 py-4">

        <NuProjectHeader project={project} />
        <NuProjectTabs projectId={projectId} active="detail" />
        ...
        {/* ── 基本情報 ───────────────────────────────────── */}
        <section className="mb-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-[var(--nu-border-soft)]">
          <h2 className="mb-3 border-b border-[var(--nu-border-soft)] pb-2 text-sm font-bold text-[var(--nu-text)]">基本情報</h2>
        ...
        <button
          type="button"
          onClick={handleSave}
          className="flex min-h-[52px] w-full items-center justify-center rounded-2xl bg-[var(--nu-primary)] px-4 py-3 text-sm font-bold text-white shadow-sm active:opacity-80"
        >
          案件を保存する
        </button>
