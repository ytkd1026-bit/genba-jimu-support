...
// 新UI 拾い出し クライアント本体（page.tsx から initialType を受け取る）
// ─────────────────────────────────────────────────────────────
...
// ─── 本体 ────────────────────────────────────────────────────
function TakeoffInner({ initialType }: { initialType: TakeoffType | null }) {
  const searchParams = useSearchParams();

  // initialType はサーバーコンポーネント（page.tsx）が searchParams から解決して渡す。
  // 本番でも request 時に SSR されるため、hydration前・JS無効でも
